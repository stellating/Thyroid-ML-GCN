...
class Engine(object):
    ...
    def train(self, data_loader, model, criterion, optimizer, epoch):
        ...
        for i, (input, target) in enumerate(data_loader):
            ...
            # print('[engine/train target]:', target)
    ...
    def validate(self, data_loader, model, criterion):
        ...
        for i, (input, target) in enumerate(data_loader):
            ...
            self.state['input'] = input
            self.state['target'] = target
            # print('[engine/validation target]:', self.state['target'])
...
class GCNMultiLabelMAPEngine(MultiLabelMAPEngine):
    ...
    def on_start_batch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        self.state['target_gt'] = self.state['target'].clone()
        # print('[engine/on_start_batch target_gt]',self.state['target'])
        self.state['target'][self.state['target'] == 0] = 1
        self.state['target'][self.state['target'] == -1] = 0
        # print('[engine/on_start_batch target]', self.state['target'])
